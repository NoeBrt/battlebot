...
def evaluate_candidate(args):
    ...
    try:
        ...
        if result.get("error"):
            return candidate_idx, -5.0, result
...
def train(args):
    ...
    for gen in range(args.generations):
        ...
        if cma.sigma < SIGMA_MIN or stagnant_gens >= args.restart_stagnation:
            ...
            print(f"  [CMA] Restart due to {why}")
        ...
        print(f"  Gen best score   : A={scoreA:.4f} B={scoreB:.4f}")
        print(f"  Gen best winRate : {winRateA:.4f}")
        print(f"  Gen best HP      : A={hpA:.2f} B={hpB:.2f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--restart-stagnation", type=int, default=10)
