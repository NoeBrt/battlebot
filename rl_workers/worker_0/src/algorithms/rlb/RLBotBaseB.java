/* ============================================================================
 * RLBotBaseB.java — Parameterized base class for RL-optimized bots.
 * All strategy constants read from RLConfigB (generated by rl_train.py).
 * ============================================================================*/
...
public abstract class RLBotBaseB extends Brain {
    ...
    // ── FIRING (parameterized via RLConfigB) ──────────────────────────────
    ...
    /** Target selection — ALL weights from RLConfigB */
    protected UEnemy chooseTarget() {
        ...
        for (UEnemy e : enemies) {
            ...
            double score = RLConfigB.TARGET_PROXIMITY_WEIGHT - d;
            ...
            else if (hp <= 100) score += RLConfigB.TARGET_LOWHP_MODERATE_BONUS;

            if (!Double.isNaN(focusX) && Math.hypot(e.x - focusX, e.y - focusY) < 120.0)
                score += RLConfigB.TARGET_FOCUS_BONUS;

            score -= e.stale * RLConfigB.TARGET_STALE_PENALTY;

            if (isSafeFire(computeLeadAngle(e))) score += RLConfigB.TARGET_SAFEFIRE_BONUS;
    ...
    protected void ageEnemies() {
        ...
        while (it.hasNext()) {
            ...
            if (e.stale > RLConfigB.STALE_TTL) it.remove();
        }
    }
    ...
    // ── POTENTIAL FIELD — parameterized via RLConfigB ──────────────────────
    ...
    protected double potentialFieldAngle(double kiteMin, double kiteMax) {
        ...
        for (UEnemy e : enemies) {
            ...
            if (dist < kiteMin) {
                double s = RLConfigB.PF_ENEMY_REPEL_STRENGTH * (kiteMin - dist) / kiteMin;
                fx -= nx * s; fy -= ny * s;
            } else if (dist > kiteMax) {
                double s = RLConfigB.PF_ENEMY_ATTRACT_STRENGTH * (dist - kiteMax) / kiteMax;
                fx += nx * s; fy += ny * s;
            } else {
                double side = M1.equals(id) ? 1.0 : (M3.equals(id) ? -1.0 : (teamA ? 1.0 : -1.0));
                fx += (-ny * side) * RLConfigB.PF_TANGENTIAL_STRENGTH;
                fy += ( nx * side) * RLConfigB.PF_TANGENTIAL_STRENGTH;
            }
        }
        ...
        for (UAlly a : allies.values()) {
            ...
            double dist = Math.hypot(dx, dy);
            if (dist < RLConfigB.PF_ALLY_REPEL_RANGE && dist > 1.0) {
                double s = RLConfigB.PF_ALLY_REPEL_STRENGTH * (RLConfigB.PF_ALLY_REPEL_RANGE - dist) / RLConfigB.PF_ALLY_REPEL_RANGE;
                fx -= (dx / dist) * s;
                fy -= (dy / dist) * s;
            }
        }

        double wf = RLConfigB.PF_WALL_STRENGTH;
        ...
        for (double[] w : wrecks) {
            ...
            double dist = Math.hypot(dx, dy);
            if (dist < RLConfigB.PF_WRECK_RANGE && dist > 1.0) {
                double s = 0.5 * (RLConfigB.PF_WRECK_RANGE - dist) / RLConfigB.PF_WRECK_RANGE;
                fx -= (dx / dist) * s;
                fy -= (dy / dist) * s;
            }
        }
