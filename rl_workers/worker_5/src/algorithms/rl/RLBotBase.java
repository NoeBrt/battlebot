/* ============================================================================
 * RLBotBase.java — Parameterized base class for RL-optimized bots.
 * All strategy constants read from RLConfig (generated by rl_train.py).
 * ============================================================================*/
...
public abstract class RLBotBase extends Brain {
    ...
    // ── FIRING (parameterized via RLConfig) ──────────────────────────────
    ...
    /** Target selection — ALL weights from RLConfig */
    protected UEnemy chooseTarget() {
        ...
        for (UEnemy e : enemies) {
            ...
            double score = RLConfig.TARGET_PROXIMITY_WEIGHT - d;
            ...
            else if (hp <= 100) score += RLConfig.TARGET_LOWHP_MODERATE_BONUS;

            if (!Double.isNaN(focusX) && Math.hypot(e.x - focusX, e.y - focusY) < 120.0)
                score += RLConfig.TARGET_FOCUS_BONUS;

            score -= e.stale * RLConfig.TARGET_STALE_PENALTY;

            if (isSafeFire(computeLeadAngle(e))) score += RLConfig.TARGET_SAFEFIRE_BONUS;
    ...
    protected void ageEnemies() {
        ...
        while (it.hasNext()) {
            ...
            if (e.stale > RLConfig.STALE_TTL) it.remove();
        }
    }
    ...
    // ── POTENTIAL FIELD — parameterized via RLConfig ──────────────────────
    ...
    protected double potentialFieldAngle(double kiteMin, double kiteMax) {
        ...
        for (UEnemy e : enemies) {
            ...
            if (dist < kiteMin) {
                double s = RLConfig.PF_ENEMY_REPEL_STRENGTH * (kiteMin - dist) / kiteMin;
                fx -= nx * s; fy -= ny * s;
            } else if (dist > kiteMax) {
                double s = RLConfig.PF_ENEMY_ATTRACT_STRENGTH * (dist - kiteMax) / kiteMax;
                fx += nx * s; fy += ny * s;
            } else {
                double side = M1.equals(id) ? 1.0 : (M3.equals(id) ? -1.0 : (teamA ? 1.0 : -1.0));
                fx += (-ny * side) * RLConfig.PF_TANGENTIAL_STRENGTH;
                fy += ( nx * side) * RLConfig.PF_TANGENTIAL_STRENGTH;
            }
        }
        ...
        for (UAlly a : allies.values()) {
            ...
            double dist = Math.hypot(dx, dy);
            if (dist < RLConfig.PF_ALLY_REPEL_RANGE && dist > 1.0) {
                double s = RLConfig.PF_ALLY_REPEL_STRENGTH * (RLConfig.PF_ALLY_REPEL_RANGE - dist) / RLConfig.PF_ALLY_REPEL_RANGE;
                fx -= (dx / dist) * s;
                fy -= (dy / dist) * s;
            }
        }

        double wf = RLConfig.PF_WALL_STRENGTH;
        ...
        for (double[] w : wrecks) {
            ...
            double dist = Math.hypot(dx, dy);
            if (dist < RLConfig.PF_WRECK_RANGE && dist > 1.0) {
                double s = 0.5 * (RLConfig.PF_WRECK_RANGE - dist) / RLConfig.PF_WRECK_RANGE;
                fx -= (dx / dist) * s;
                fy -= (dy / dist) * s;
            }
        }
