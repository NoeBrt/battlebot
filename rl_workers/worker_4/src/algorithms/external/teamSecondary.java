...
;
  ...
  public void step() {
	...
	if (state==FIRSTMOVETASK) {
		myMove(); 
		if (whoAmI == MARIO) {
			if ((myTeam && myY > 1350) || (!myTeam && myY > 1350)) { 
				if (myTeam) {
					state=TURNEASTTASK;	    			  
				} else {
					state = TURNWESTTASK;
    ...
    if (state==MOVETASK) {
        if (detectFront().getObjectType() == IFrontSensorResult.Types.WALL) {    
            // Both robots have similar behavior initially - but MARIO turns left while ROCKY turns right
            if (stepNumber < 1000) { // Initial phase - MARIO mirrors ROCKY's behavior but turns left
                if (whoAmI == MARIO) {
                    state = TURNLEFTTASK;
                ...
                return;
            } else { // After initial phase, resume original behaviors
                if (whoAmI == MARIO) {
                    // MARIO's original wall avoidance logic
                ...
                } else { // ROCKY - same behavior throughout the game
                    state=TURNRIGHTTASK;
                    endTaskDirection = getHeading() + Parameters.RIGHTTURNFULLANGLE;
                    stepTurn(Direction.RIGHT);
                    return;
