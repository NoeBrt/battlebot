/* ============================================================================
 * RLBotBaseB.java — RL-optimized base (extends MacDuoBaseBot).
...
public abstract class RLBotBaseB extends MacDuoBaseBot {
    ...
    protected RLEnemy chooseBestTarget() {
        ...
        for (RLEnemy e : rlEnemies) {
            ...
            score += (RLConfigB.TARGET_PROXIMITY_WEIGHT - dist);
            if (e.type == Types.OpponentSecondaryBot) score += RLConfigB.TARGET_TYPE_BONUS;

            double hp = (e.type == Types.OpponentSecondaryBot ? 100 : 300) - e.estimatedDmg;
            if (hp < 30) score += RLConfigB.TARGET_LOWHP_CRITICAL_BONUS;
            else if (hp < 60) score += RLConfigB.TARGET_LOWHP_MODERATE_BONUS;

            // Safe Fire (simplified check)
            if (isFiringLineSafe(e.x, e.y)) score += RLConfigB.TARGET_SAFEFIRE_BONUS;

            int stale = currentTick - e.lastSeenTick;
            score -= (stale * RLConfigB.TARGET_STALE_PENALTY);
    ...
    protected void potentialFieldMove(double kiteMin, double kiteMax) {
        ...
        for (RLEnemy e : rlEnemies) {
            ...
            double force = 0;
            if (d < kiteMin) force = RLConfigB.PF_ENEMY_REPEL_STRENGTH * (kiteMin - d) / 100.0; 
            else if (d > kiteMax) force = -RLConfigB.PF_ENEMY_ATTRACT_STRENGTH * (d - kiteMax) / 100.0;
            
            fx += (dx/d) * force;
            fy += (dy/d) * force;
            
            // Tangential
            fx += -(dy/d) * RLConfigB.PF_TANGENTIAL_STRENGTH;
            fy += (dx/d) * RLConfigB.PF_TANGENTIAL_STRENGTH;
        }
        ...
        for (BotState b : allyPos.values()) {
             ...
             double d = Math.hypot(dx, dy);
             if (d < RLConfigB.PF_ALLY_REPEL_RANGE && d > 0) {
                 double force = RLConfigB.PF_ALLY_REPEL_STRENGTH * (RLConfigB.PF_ALLY_REPEL_RANGE - d);
                 fx += (dx/d) * force;
                 fy += (dy/d) * force;
             }
        }
        ...
        // 3. Wall Repel
        double wallD = 200;
        if (myPos.getX() < wallD) fx += RLConfigB.PF_WALL_STRENGTH * (wallD - myPos.getX());
        if (myPos.getX() > 3000 - wallD) fx -= RLConfigB.PF_WALL_STRENGTH * (myPos.getX() - (3000 - wallD));
        if (myPos.getY() < wallD) fy += RLConfigB.PF_WALL_STRENGTH * (wallD - myPos.getY());
        if (myPos.getY() > 2000 - wallD) fy -= RLConfigB.PF_WALL_STRENGTH * (myPos.getY() - (2000 - wallD));
        ...
        for (double[] w : wreckPositions) {
            ...
            double d = Math.hypot(dx, dy);
            if (d < RLConfigB.PF_WRECK_RANGE && d > 0) {
                double force = RLConfigB.PF_WALL_STRENGTH * (RLConfigB.PF_WRECK_RANGE - d);
                fx += (dx/d) * force;
                fy += (dy/d) * force;
            }
        }
